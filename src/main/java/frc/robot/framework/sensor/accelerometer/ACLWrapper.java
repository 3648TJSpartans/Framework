...
import frc.robot.framework.sensor.accelerometer.ACLTypes.ADXL_345_I2C;
import frc.robot.framework.sensor.accelerometer.ACLTypes.ADXL_345_SPI;
import frc.robot.framework.sensor.accelerometer.ACLTypes.ADXL_362;
import frc.robot.framework.sensor.accelerometer.ACLTypes.Analog_Accelerometer;
import frc.robot.framework.sensor.accelerometer.ACLTypes.Built_In_Accelerometer;
...
public class ACLWrapper implements ACLBase {
    ...
    public ACLWrapper(Element element) {
        ...
        if (m_accelerometer == null) {
            System.out.println("For motor: " + id + " ACL type: " + m_accelerometerElement.getAttribute("controller")
                    + " was not found!");
            return;
        }
    }
