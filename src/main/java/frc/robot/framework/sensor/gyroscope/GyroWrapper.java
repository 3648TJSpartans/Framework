...
import frc.robot.framework.sensor.gyroscope.GyroTypes.Analog_Gyro;
import frc.robot.framework.sensor.gyroscope.GyroTypes.Pigeon;
import frc.robot.framework.sensor.gyroscope.GyroTypes.nav_X;
...
public class GyroWrapper implements GyroBase {
    ...
    public GyroWrapper(Element element) {
        ...
        if (m_gyro == null) {
            System.out.println(
                    "For motor: " + id + " gyro type: " + m_gyroElement.getAttribute("controller") + " was not found!");
            return;
        }
